    ...
    try {
      ...
      if (finalSelected.length < 10) {
        ...
        try {
          ...
          this.logger.warn(
            `LLM supplement failed, returning ${finalSelected.length} POI`,
          );
        }
      }
...
Верни ТОЛЬКО JSON без markdown:
{
  ...
  private buildPrompt(pois: PoiItem[], intent: ParsedIntent): string {
    ...
    const hasNightlifeFocus = /ночн|клуб|бар|вечер|развлечени|весели/i.test(
      preferences,
    );
...
${
  hasFoodFocus
    ? `9. 🔴 CRITICAL FOOD PRIORITY (TRI-108-6 Extended):
...
Если нужно выбрать рестораны - бери дешевые и середину по цене, избегай дорогих.`;
  ...
  private buildQuantityConstraints(
    ...
    if (minRestaurants) {
      constraints.push(
        `ОБЯЗАТЕЛЬНО включи минимум ${minRestaurants} ресторанов`,
      );
    }
    if (minCafes) {
