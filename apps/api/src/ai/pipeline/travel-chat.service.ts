...
- Предлагай конкретные изменения: "Могу добавить кафе, удалить ненужные точки, заменить место или сделать маршрут дешевле — просто скажи что хочешь."`;
...
4. Можно: удалять точки, менять их порядок, переносить между днями.
5. Нельзя: добавлять новые poi_id, которых нет в исходном маршруте.
6. Если пользователь хочет ДОБАВИТЬ что-то новое (новое кафе, новый музей, точку, место, достопримечательность) — верни action="chat" с советом написать "Добавь [категория]". Слова "добавь", "добавить", "включи", "добавь точку", "добавь место" — ВСЕГДА action="chat", НИКОГДА не возвращай action="modify" с пустым days[].
7. Если запрос непонятен — верни action="chat" с уточняющим вопросом.
8. Если пользователь ЯВНО просит удалить весь маршрут / очистить всё / убрать все точки ("удали маршрут", "очисти", "удали всё") — верни action="modify" с пустым массивом days: []. НЕ применяй это правило к запросам на добавление.
  ...
  async modifyRoute(
    ...
    try {
      ...
      if (parsed.action === 'modify' && Array.isArray(parsed.days)) {
        ...
        // Safety guard: never clear the route if user was asking to ADD something
        const isAddRequest = /добав|включи|append|add/i.test(userMessage);
        if (parsed.days.length === 0) {
          if (isAddRequest) {
            // LLM hallucinated a clear-all — treat as chat instead
            return {
              type: 'chat',
              message: parsed.message ?? 'Уточни, пожалуйста, какое место хочешь добавить в маршрут?',
            };
          }
          return {
            type: 'modify',
