...
export class PointMutationService {
  ...
  async applyMutations(
    ...
    console.log(
      `[PointMutationService] Applying ${mutations.length} mutations to trip ${tripId}`,
    );
    return await this.db.transaction(async (tx) => {
      ...
      if (trip.version !== ifMatch) {
        throw new BadRequestException(
          'Version conflict - please refresh your client',
        );
      }
      ...
      const resolvedMutations = await this.resolveReferences(
        mutations,
        currentPoints,
      );
      console.log(
        `[PointMutationService] Resolved mutations:`,
        JSON.stringify(resolvedMutations),
      );

      // 4. Apply mutations to memory state
      const mutatedPoints = applyMutations(
        currentPoints,
        resolvedMutations,
        (m) => ({
          id: randomUUID(),
      ...
      console.log(
        `[PointMutationService] Syncing points to DB. Current count: ${currentPoints.length}, New count: ${updatedPoints.length}`,
      );
      ...
      // 7. Broadcast update
      this.logger.log(
        `Broadcasting trip_version_updated for trip ${tripId}, version ${newVersion}, points: ${updatedPoints.length}`,
      );
  ...
  async applyMutationsInMemory(
    mutations: PointMutation[],
    points: DbRoutePoint[],
  ): Promise<DbRoutePoint[]> {
  ...
  async resolveReferences(
    ...
    this.logger.log(
      `Resolving references among points: ${points.map((p) => p.title).join(', ')}`,
    );
    ...
    for (const mutation of mutations) {
      if (mutation.type === 'REMOVE_BY_QUERY') {
        ...
        const queryNormJoined = mutation.query
          .toLowerCase()
          .replace(/[^а-яёa-z0-9]/g, '');
        ...
        const matches = points.filter((p) => {
          ...
          if (titleNormJoined === queryNormJoined && queryNormJoined.length > 0)
            return true;
        ...
        if (matches.length > 0) {
          const toRemove =
            mutation.limit === 1 ? [matches[0].id] : matches.map((m) => m.id);
          resolved.push({
            type: 'REMOVE_BY_ID',
            pointIds: toRemove,
          });
  ...
  private async syncPointsToDb(
    tx: any,
    oldPoints: DbRoutePoint[],
    newPoints: DbRoutePoint[],
  ) {
