...
export function applyMutation<T extends BaseRoutePoint>(
  ...
  switch (mutation.type) {
    case 'ADD': {
      if (!pointBuilder)
        throw new Error('pointBuilder required for ADD mutation');
      const newPoint = pointBuilder(mutation);
      ...
      return [...points.slice(0, idx + 1), newPoint, ...points.slice(idx + 1)];
    }
    case 'REMOVE_BY_ID':
...
export function applyMutations<T extends BaseRoutePoint>(
  ...
  return mutations.reduce(
    (acc, m) => applyMutation(acc, m, pointBuilder),
    points,
  );
}
