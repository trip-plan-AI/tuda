...
# Version: 2.1 | Status: MVP | Date: 2026-03-07
...
| Изменение | Причина |
|-----------|---------|
| STEP 2 переведён с Yandex Search API на бесплатный Provider Search Engine KudaGo + Overpass | Отказ от платного `search-maps.yandex.ru/v1`, снижение стоимости и расширение покрытия POI |
| Добавлен порядок источников: сначала KudaGo, затем добор через Overpass | KudaGo даёт более «вкусные» точки для туризма, Overpass закрывает дефицит по количеству |
| В STEP 2 введён source-aware merge и дедупликация между провайдерами | Исключение дублей и контроль качества объединённого набора |
| Обновлены failure-сценарии STEP 2 для двух провайдеров | Нужна прозрачная деградация при частичной недоступности внешних API |
| Обновлены NFR по latency и fallback для нового поискового контура | Актуализация SLA и поведения системы под новую архитектуру |
...
> Комментарий к изменению v2.1: в рамках задачи cost-оптимизации и отказа от платного API в STEP 2 используется только бесплатный backend-провайдер поиска POI: сначала KudaGo, затем Overpass как добор.
...
- Обеспечить региональную релевантность и полноту POI через KudaGo + Overpass
...
[GPT-4o-mini: Orchestrator]           timeout: 20s
 ...
 │
 ▼ ─────────────────────────────────────── STEP 2 ──
[NestJS: Provider Search Engine]      timeout: 10–12s per-request
 │   Вход: ParsedIntent
 │   1. HTTP → KudaGo API (приоритетный источник «вкусных» мест)
 │   2. Если не хватает POI: HTTP → Overpass API (добор по категориям)
 │   3. Нормализация в единый PoiItem[] + source metadata
 │   4. Валидация координат: lat ∈ [-90, 90], lon ∈ [-180, 180]
 │   5. Дедупликация (< 50 м — дубль, оставить max rating/quality)
 │   6. Pre-filter: исключения + сортировка + обрезка до 15 POI
...
| **NestJS: Provider Search** | HTTP в KudaGo/Overpass, merge, валидация, pre-filter | Принимать продуктовые решения |
...
### FR-03: STEP 2 — Provider Search Engine (NestJS, KudaGo + Overpass)

NestJS выполняет HTTP-запросы самостоятельно. LLM не участвует.

> Комментарий к изменению v2.1: ранее STEP 2 опирался на платный Yandex Search API. Теперь используется бесплатный контур из двух источников.

1. `Promise.all` — параллельные запросы в KudaGo по категориям/тематике,
   timeout 8–10s на каждый запрос
2. Если после KudaGo < `MIN_POI_BEFORE_SEMANTIC`, выполнить добор через Overpass,
   timeout до 12s на запрос
3. Нормализация каждого объекта в `PoiItem` с полем `source` (`kudago` | `overpass`)
4. **Валидация координат** — обязательна; при провале — POI отбрасывается:
   - `lat` ∈ `[-90, 90]` — число, не null/NaN/string
   - `lon` ∈ `[-180, 180]` — число, не null/NaN/string
5. **Source-aware дедупликация:** `Haversine(a, b) < 0.05 км` → дубль;
   приоритет: KudaGo c рейтингом/описанием > Overpass
6. **Pre-filter до 15 POI:**
   - Убрать POI из `excluded_categories`
   - Сортировать по quality score (`rating`, полнота метаданных, source priority)
   - Обрезать до 15 объектов
7. Если осталось < 3 POI — повторить Overpass с `radius_km * 1.3` (max 1 retry)
...
| STEP 2 timeout (Provider Search) | **8–12 сек** per-request | Fallback: Redis cache → partial/HTTP 422 |
...
└────────────┬─────────────────────────────────┬───────────────────┘
     ...
     [STEP 2]                          [STEP 3]
     NestJS                            YandexGPT
      Provider Search Engine            Semantic Filter
...
│   ├── provider-search.service.ts      # STEP 2: KudaGo + Overpass + merge + filter
│   ├── kudago-client.service.ts        # STEP 2a: HTTP client KudaGo
│   ├── overpass-client.service.ts      # STEP 2b: HTTP client Overpass
...
async createPlan(userId: string, dto: AiPlanRequestDto): Promise<AiPlanResponse> {
  ...
  const rawPoi   = await this.providerSearch.fetchAndFilter(intent);                // STEP 2
...
| STEP 2 Provider Search | **8–12 сек** per-request | Redis cache → partial/HTTP 422 если нет кэша |
...
**[F-02] KudaGo недоступен, Overpass доступен (STEP 2)**
```
Условие: Timeout/HTTP ошибка от KudaGo
Действие:
  1. Продолжить STEP 2 только с Overpass
  2. Пометить fallback: 'KUDAGO_UNAVAILABLE_OVERPASS_ONLY'
  3. Если итоговых POI >= 3 — продолжить пайплайн
```

**[F-02b] KudaGo и Overpass недоступны (STEP 2)**
```
Условие: оба провайдера не вернули валидные данные
Действие:
  1. Проверить Redis по ключу: SHA256(city + categories.sort().join(',') + radius_km)
  2. Cache hit (TTL 24h) → вернуть cached PoiItem[], fallback: 'PROVIDER_CACHE_HIT'
  3. Cache miss → poi_list = [], fallback: 'PROVIDER_SEARCH_UNAVAILABLE' → HTTP 422
```
